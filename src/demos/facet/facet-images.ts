...
function loadImage(src: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () =>
      reject(new FacetImageError('Could not read this image file.'))
    img.src = src
  })
}
...
export async function processImageFile(
  ...
  if (!isImageFile(file)) {
    throw new FacetImageError(
      'Please choose a JPG, PNG, GIF, WebP, or other image file.',
    )
  }

  if (file.size > 12 * 1024 * 1024) {
    throw new FacetImageError(
      'Image is too large. Please use a file under 12 MB.',
    )
  }
